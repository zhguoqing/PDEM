...
from utils import *
from loss import OriTripletLoss, TripletLoss_WRT, KLDivLoss, TripletLoss_ADP, SCLoss, hetero_loss, DCLoss
...
if dataset == 'sysu':
    ...
    data_path = '/home/student/File/wangzhun/Datasets/RegDB/'
    log_path = args.log_path + 'regdb_log/'
    test_mode = [2, 1]  # visible to thermal
elif dataset == 'llcm':
    data_path = '/home/student/File/wangzhun/Datasets/LLCM/'
...
def train(epoch):
    ...
    for batch_idx, (input10, input11, input2, label1, label2) in enumerate(trainloader):
        ...
        feat0, feat1, feat2 = feat.chunk(3,0)
        loss_dcl = criterion_DCL(torch.cat((feat0, feat1), 0), label1) * 0.4 + criterion_DCL1(torch.cat((feat0, feat2), 0), label1) * 0.2
        loss_hc = criterion_hc(feat0, feat2, label1, label1)
        ...
        loss_tri = (criterion_tri(torch.cat((feat0, feat1),0), lbs)[0] + criterion_tri(torch.cat((feat0, feat2),0), lbs)[0] + criterion_tri(torch.cat((feat1, feat2),0), lbs)[0])/3
        batch_acc = criterion_tri(torch.cat((feat0, feat2),0), lbs)[1]
        correct += (batch_acc / 2)
        _, predicted = out0.max(1)
        correct += (predicted.eq(labels).sum().item() / 2)
        
        # pdb.set_trace()
        loss = loss_id + loss_tri + args.kl * loss_kl + loss_dcl + loss_hc
        ...
        hc_loss.update(loss_hc, 2 * input1.size(0))
        total += labels.size(0)
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'HCLoss: {hc_loss.val:.4f} ({hc_loss.avg:.4f}) '
                  'Accu: {:.2f}'.format(
                epoch, batch_idx, len(trainloader), current_lr,
                100. * correct / total, batch_time=batch_time,
                train_loss=train_loss, id_loss=id_loss, tri_loss=tri_loss,kl_loss=kl_loss,dcl_loss=dcl_loss, hc_loss=hc_loss))
